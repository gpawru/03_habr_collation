...
/// читаем следующий символ (только для well-formed UTF-8 строк)
#[inline(always)]
pub unsafe fn next_char(iter: &mut CharsIter) -> Option<u32>
{
    match !iter.is_empty() {
        true => Some(next_char_unchecked(iter)),
        false => None,
    }
}

/// читаем следующий символ (без проверки)
#[inline(always)]
pub unsafe fn next_char_unchecked(iter: &mut CharsIter) -> u32
{
    let x = char_first_byte_unchecked(iter);

    if x < 0x80 {
        return x as u32;
    }

    char_nonascii_bytes_unchecked(iter, x)
}
